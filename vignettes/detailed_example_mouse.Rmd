...
Human-based methods can still be used to deconvolve mouse data through the use of orthologi genes. The function `mouse_genes_to_human` does that by retrieving the correspondent gene names with `biomaRt`. Since the genes are retrieved from an online database the function could produce some errors if the connection is not stable. 


```{r, message=FALSE, warning=FALSE}

dataset_petitprez_humanGenes <- mouse_genes_to_human(dataset_petitprez$expr_mat)
