...
quanTIseq provides scores for CD4+ and CD8+ T cells independently, but you are
interested in the fraction of overall T cells. In that case you can use
`map_result_to_celltypes` to sum up the scores:

```{r, message=FALSE}
res = deconvolute(immunedeconv::example_gene_expression_matrix, "quantiseq") %>%
  map_result_to_celltypes(c("T cell"), "quantiseq")
knitr::kable(res, digits=2)
```
...
You can, however, provide custom signatures for most individual methods.
